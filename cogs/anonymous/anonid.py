...
idLength = 12 #the lenght of the public id

def idGenerator(length: int = idLength):
    characters = string.ascii_letters + string.digits
    return "".join(secrets.choice(characters) for _ in range(length))

async def idVerification(conn: aiosqlite.Connection):
    while True:
        newId = idGenerator()
...
class AnonId(commands.Cog):
    ...
    async def anonid(self, ctx: commands.Context[commands.Bot]):
        ...
        if not existingId:
            newId = await idVerification(conn) #creates an id
        ...
        elif len(existingId[0]) != idLength:
            newId = await idVerification(conn) #creates another id with updated length
        ...
        resultEmbed = discord.Embed(
            title = "Anonymous ID",
            description = (
                f"Your anonymous ID is: `{existingId[0] if existingId and not newId else newId}`"
    ...
    @app_commands.command(
        ...
        description = "Shows the anonymous ID for the user.",
         extras = {"Category": "Anonymous", "in-server": "Yes"}
    )
    ...
    async def slashAnonid(self, interaction: discord.Interaction):
        ...
        #checks if id length is ok
            ...
            newId = await idVerification(conn) #creates another id with updated lenght
