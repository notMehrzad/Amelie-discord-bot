...
class Balance(commands.Cog):
    ...
    @balance.error
    async def balance_error(self, ctx: commands.Context[commands.Bot], error: Exception):
    ...
    @slashBalance.error
    async def slashBalance_error(self, interaction: discord.Interaction, error: Exception):
        logger.exception(f"❌ something went wrong with /balance command:")
        try:
            await interaction.response.send_message("something went wrong with **balance**.", ephemeral = True)
        except discord.InteractionResponded:
            await interaction.followup.send("something went wrong with **balance**.", ephemeral = True)
