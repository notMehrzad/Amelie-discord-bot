...
class CoinFlip(commands.Cog):
    ...
    async def slashCoinflip_error(self, interaction: discord.Interaction, error: Exception):
        logger.exception(f"❌ something went wrong with /coinflip command:")
        try:
            await interaction.response.send_message("something went wrong with **coinflip**.", ephemeral = True)
        except discord.InteractionResponded:
            await interaction.followup.send("something went wrong with **coinflip**.", ephemeral = True)
