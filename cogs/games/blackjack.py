...
class Blackjack(commands.Cog):
    ...
    async def blackjack(self, ctx: commands.Context[commands.Bot], bet: int | str | None = None):
        #if player bets
        if bet:
            ...
            if ctx.author.id in sessions:
                return await ctx.reply("You have an actice game session somewhere. close that one first and try again.")
            
            #if bet is lower than limit
            if bet < 100:
                return await ctx.reply("The minimum bet limit for this game is **100**.")
        ...
        view = blackjackView(ctx, bet if isinstance(bet, int) else 0) #initializes the view
        await view.start()

    @blackjack.error
    async def blackjack_error(self, ctx: commands.Context[commands.Bot], error: commands.CommandError):
    ...
    @app_commands.describe(bet = "The amount you want to set your initial bet. (max: 300/min: 100)")
    async def slashBlackjack(self, interaction: discord.Interaction, bet: int | None = None):
        #if player bets
        if bet:
            #if player has an active session
            if interaction.user.id in sessions:
                return await interaction.response.send_message("You have an actice game session somewhere. close that one first and try again.", ephemeral = True)
            
            #if bet is lower than limit
            if bet < 100:
                return await interaction.response.send_message("The minimum bet limit for this game is **100**.", ephemeral = True)
            #if bet is higher than limit
            elif bet > 300:
        ...
        view = blackjackView(interaction, bet if isinstance(bet, int) else 0) #initializes the view
        await view.start()

    @slashBlackjack.error
    async def slashBlackjack_error(self, interaction: discord.Interaction, error: Exception):
        logger.exception(f"❌ something went wrong with /blackjack command:")
        try:
            await interaction.response.send_message("something went wrong with **blackjack**.", ephemeral = True)
        except discord.InteractionResponded:
            await interaction.followup.send("something went wrong with **blackjack**.", ephemeral = True)

class blackjackView(discord.ui.View):
    def __init__(self, ctx: commands.Context[commands.Bot] | discord.Interaction, bet: int):
    ...
    async def insuranceOffer(self):
            ...
            if self.slash:
                await self.interaction.edit_original_response(embed = insuranceEmbed, view = self)
            else:
                await self.msg.edit(embed = insuranceEmbed, view = self)

    @discord.ui.button(label = "refuse", style = discord.ButtonStyle.red)
    async def refuse(self, interaction: discord.Interaction, button: discord.ui.Button[discord.ui.View]):
    ...
    async def accept(self, interaction: discord.Interaction, button: discord.ui.Button[discord.ui.View]):
        if interaction.user.id != self.user.id:
            return await interaction.response.send_message("You can't play in this match.", ephemeral = True)
    ...
    async def hit(self, interaction: discord.Interaction, button: discord.ui.Button[discord.ui.View]):
        if interaction.user.id != self.user.id:
            return await interaction.response.send_message("You can't play in this match.", ephemeral = True)
        
        self.surrender.disabled = self.doubledown.disabled = True #disables surrender and double down buttons
    ...
    async def doubledown(self, interaction: discord.Interaction, button: discord.ui.Button[discord.ui.View]):
        if interaction.user.id != self.user.id:
            return await interaction.response.send_message("You can't play in this match.", ephemeral = True)
    ...
    async def surrender(self, interaction: discord.Interaction, button: discord.ui.Button[discord.ui.View]):
        if interaction.user.id != self.user.id:
            return await interaction.response.send_message("You can't play in this match.", ephemeral = True)
        ...
        if self.slash:
                await self.interaction.edit_original_response(embed = surrenderEmbed, view = None)
        else:
    ...
    async def dealersTurn(self):
        ...
        if self.slash:
                await self.interaction.edit_original_response(embed = dealerTurnEmbed, view = None)
        else:
