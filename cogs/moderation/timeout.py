...
class Timeout(commands.Cog):
    ...
    async def slashTimeout(self, interaction: discord.Interaction, user: discord.Member, until: str, reason: str | None = None):
        #if user runs the command in dm
        ...
        if user.is_timed_out() and until.lower() not in ["false", "0", "remove"]:
            return await interaction.response.send_message(f"{user.display_name} is timed out already.", ephemeral = True)
        
        #if entered time was 0, removes the timeout
        if until.lower() in ["false", "0", "remove"]:
            if user.is_timed_out():
                await user.timeout(None, reason = reason)
                await interaction.response.send_message(f"{user.display_name}'s timeout has been *removed* via {interaction.user.display_name}." + (f"\nreason: {reason}" if reason else ""))
            else:
                await interaction.response.send_message(f"{user.display_name} had not been timed out at the first place.", ephemeral = True)
            return
        else:
        ...
        try:
            ...
            if isinstance(parsedUntil, timedelta):
                await interaction.response.send_message(f"{user.display_name} has been *timed out* via {interaction.user.display_name} for `{until}`." + (f"\nreason: {reason}" if reason else ""))
            else:
                await interaction.response.send_message(f"{user.display_name} has been *timed out* via {interaction.user.display_name} until `{until}` ." + (f"\nreason: {reason}" if reason else ""))
