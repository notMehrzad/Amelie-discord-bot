...
class TicketHandle(commands.Cog):
    ...
    async def tickethandle(self, ctx: commands.Context[commands.Bot], cmd: str | None, ticketId: int | str | None):
        ...
        if ctx.author.id in sessions:
            return await ctx.reply("You have an open *Ticket Responding* session. Try closing it and try again.")
        ...
        try:
            ticketUser = self.bot.get_user(row["user_id"]) or await self.bot.fetch_user(row["user_id"])
        ...
        if cmd == "respond":
            ...
            view = TicketRespondView(ctx, ticketId, ticketUser, messageCollector, subject) #initializes the ticket responding view
            await view.start()
...
class TicketRespondView(discord.ui.View):
    def __init__(self, ctx: commands.Context[commands.Bot], ticketId: int, ticketUser: discord.User, messageCollector: discord.Message | None, subject: str):
    ...
    async def start(self):
        ...
        self.msg = await self.ctx.reply(embed = initialEmbed, view = self) #sends the initial message
    ...
    async def done(self, interaction: discord.Interaction, button: discord.ui.Button[discord.ui.View]):
        ...
        messages = sessions.pop(self.user.id) #ends the admin responding session and collects the sent messages
    ...
    async def on_error(self, interaction: discord.Interaction, error: Exception, item: discord.ui.Item[discord.ui.View]):
        logger.exception(f"❌ something went wrong with tickethandle interaction - button: {getattr(item, 'label', 'unknown')}")
        try:
            await interaction.response.send_message("something went wrong with **tickethandle**.", ephemeral = True)
        except discord.InteractionResponded:
            await interaction.followup.send("something went wrong with **tickethandle**.", ephemeral = True)
        except Exception:
            pass
